...
||
|| $Log: 20160520_udr_structure.sql,v $
...
DECLARE
	...
	c_filename	CONSTANT VARCHAR2(100) := '20160520_udr_structure.sql';
...
WHERE	partition_template_id IN
	...
	START	WITH UDS_CONTEXT_ID IN
		...
		SELECT	UDS_CONTEXT_ID
		FROM	UDS_CONTEXT
		WHERE	UDS_CONTEXT_KEY IN ('RTX_ST')
		)
	CONNECT	BY parent_partition_id = PRIOR partition_template_id
	);
...
LOOP
	...
	FROM	uds_context	c,
		uds_tables	t
	WHERE	c.uds_context_key = 'RTX_ST'
