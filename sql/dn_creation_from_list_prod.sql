/*
|| Script to create directory numbers in production from the list provided
|| by TSTT
|| Created by Serge Shmygelskyy aka Shmyg
|| mailto: serge.shmygelskyy@gmail.com
||
|| $Log: dn_creation_from_list_prod.sql,v $
|| Revision 1.2  2016/09/22 13:31:09  shmyg
|| *** empty log message ***
||
|| Revision 1.1  2016/09/19 09:55:01  shmyg
...
BEGIN
	...
	SELECT	v_next_free_value + ROWNUM - 1,
		...
		c_business_unit_id,
		c_rscode
	FROM	shmyg.dn_list	dn,
		mpdhmtab	mp
	WHERE	dn.area = mp.shdes
	AND	dn.dn_num NOT IN 
		(
		SELECT	dn_num
