...
<!--
  ...
  File:    CallDetailStatementFO.xsl
  ...
  VERSION = @(#) ./lhsj_main/bscs/batch/src/bgh/LAYOUT/CallDetailStatementFO.xsl, , CBIO_PCP1503, CBIO_PCP1503_150728 @@CBIO_PCP1503_150728 28-Jul-2015
...
<xsl:stylesheet version="1.0" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:cur="http://www.unece.org/etrades/unedocs" xmlns:set="http://exslt.org/sets" exclude-result-prefixes="xsl cur set">
            ...
            <fo:table-body>
              ...
              <fo:table-row>
                <fo:table-cell>
                  <fo:block font-size="8pt" text-align="left">
                    <xsl:for-each select="$txt">
                      <xsl:value-of select="key('txt-index','416')[@xml:lang=$lang]/@Des"/>
                    </xsl:for-each>
                  </fo:block>
                </fo:table-cell>
              ...
              <fo:table-row>
                <fo:table-cell>
                  <fo:block font-size="8pt" text-align="left">
                    <xsl:for-each select="$txt">
                      <xsl:value-of select="key('txt-index','417')[@xml:lang=$lang]/@Des"/>
                    </xsl:for-each>
                  </fo:block>
                </fo:table-cell>
                <fo:table-cell font-size="8pt" text-align="right">
  ...
  <xsl:template name="CallsPerPeriod">
    ...
    <!-- period info -->
    <fo:table space-before="3mm" width="160mm" table-layout="fixed">
      <fo:table-column column-width="80mm" column-number="1"/>
      <fo:table-column column-width="80mm" column-number="2"/>
        <fo:table-body>
          <fo:table-row>
            <fo:table-cell>
              <fo:block font-weight="bold" font-size="7pt" text-align="left">
                <xsl:for-each select="$txt">
            ...
            <fo:table-cell>
              <fo:block font-weight="bold" font-size="7pt" text-align="right">
                <!-- period start -->
                <xsl:if test="Date[@Type='START']">
                  <xsl:call-template name="date-time-format">
